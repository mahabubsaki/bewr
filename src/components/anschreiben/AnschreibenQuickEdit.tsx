...
function SI(props: React.ComponentProps<typeof Input>) {
  return (
    <Input {...props} className={`h-11 rounded-xl border-border/60 bg-background focus-visible:border-primary/50 focus-visible:ring-primary/15 text-sm ${props.className ?? ""}`} />
  );
}

export default function AnschreibenQuickEdit({ open, onOpenChange, data, update }: Props) {
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="w-[95vw] max-w-xl overflow-hidden rounded-2xl border-none p-0 shadow-2xl sm:w-full">
        <div className="bg-linear-to-br from-emerald-500/10 via-background to-background p-5 sm:p-7">
          <DialogHeader className="pb-5 text-left">
            <div className="mb-2 flex items-center gap-3">
              <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-linear-to-br from-emerald-500 to-teal-600 text-white shadow-lg shadow-emerald-500/30">
          ...
          <div className="grid gap-4 max-h-[58vh] overflow-y-auto pb-1">
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <Field label="Straße & Hausnr.">
                <SI value={data.recipientStreet} onChange={(e) => update("recipientStreet", e.target.value)} placeholder="z.B. Beispielweg 10" />
              </Field>
              <Field label="PLZ & Stadt">
                <SI value={data.recipientCity} onChange={(e) => update("recipientCity", e.target.value)} placeholder="z.B. 54321 Berlin" />
              </Field>
            </div>
            <Field label="Job Position / Betreff">
              <SI value={data.subject} onChange={(e) => update("subject", e.target.value)} placeholder="Bewerbung als…" className="font-bold italic" />
            </Field>
            <div className="rounded-xl border border-emerald-200/60 bg-emerald-50/60 p-3 text-xs text-emerald-800/80 dark:bg-emerald-950/20 dark:text-emerald-300">
              💡 <strong>Tipp:</strong> Anrede wird automatisch gesetzt, wenn du
              „Herr Müller" oder „Frau Schmidt" eingibst.
            </div>
          </div>
          ...
          <DialogFooter className="mt-5 gap-2 border-t pt-5">
            ...
            <Button className="rounded-full px-8 shadow-lg shadow-emerald-500/20" onClick={() => onOpenChange(false)}>
              Übernehmen
            </Button>
          </DialogFooter>
        </div>
