...
export default function AnschreibenPage() {
  ...
  const handleSyncFromDeckblatt = () => {
    try {
      const deckblattStr = localStorage.getItem("deckblatt");
      if (deckblattStr) {
        const deckblattData: DeckblattData = JSON.parse(deckblattStr);
        setData((prev: AnschreibenData) => ({
          ...prev,
          senderName: deckblattData.name || prev.senderName,
          senderStreet: deckblattData.street || prev.senderStreet,
          senderCity: deckblattData.city || prev.senderCity,
          senderPhone: deckblattData.phone || prev.senderPhone,
          senderEmail: deckblattData.email || prev.senderEmail,
          senderNameClosing: deckblattData.name || prev.senderNameClosing,
        }));
      ...
      console.error("Error syncing from deckblatt:", error);
    ...
    <div className="flex min-h-screen flex-col bg-background selection:bg-primary/10">
      ...
      <div className="sticky top-0 z-30 flex flex-col gap-4 border-b bg-background/80 px-4 py-4 backdrop-blur-md sm:flex-row sm:items-center sm:justify-between sm:px-6">
        <div className="space-y-1">
          ...
          <div className="flex items-center gap-3">
            ...
            <Badge
              variant="secondary"
              className="bg-primary/10 text-primary-foreground pointer-events-none uppercase text-[10px] font-bold"
            >
              Premium UI
            </Badge>
          </div>
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          <Button
            ...
            className="flex-1 sm:flex-none gap-2 border-primary/20 hover:bg-primary/5 transition-all shadow-sm"
          >
          ...
          <Button
            ...
            className="flex-1 sm:flex-none gap-2 bg-primary shadow-lg shadow-primary/20 hover:scale-[1.02] active:scale-95 transition-all"
          >
            <Download size={18} />{" "}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-0 overflow-hidden bg-muted/30 lg:rounded-2xl border-y lg:border shadow-inner">
        ...
        <div className="lg:col-span-5 h-auto overflow-y-visible lg:overflow-y-auto border-t lg:border-t-0 lg:border-r bg-background/50 backdrop-blur-sm order-2 lg:order-1">
          <Tabs defaultValue="content" className="w-full">
            <div className="sticky top-0 z-20 bg-background/95 backdrop-blur supports-backdrop-filter:bg-background/60 py-2 border-b px-4">
              <TabsList className="w-full grid grid-cols-4 h-11 p-1 bg-muted/30">
                <TabsTrigger
                  value="content"
                  className="gap-2 rounded-md transition-all text-xs"
                >
                  <Edit3 size={14} /> Text
                </TabsTrigger>
                <TabsTrigger
                ...
                >
                  <User size={14} /> Absender
                </TabsTrigger>
                <TabsTrigger
                  value="recipient"
                  className="gap-2 rounded-md text-xs"
                >
                  <Building2 size={14} /> Empfänger
                </TabsTrigger>
                <TabsTrigger
                  value="settings"
                  className="gap-2 rounded-md text-xs"
            ...
            <div className="px-4 py-6 space-y-8 pb-32">
              <TabsContent value="content" className="m-0 space-y-6">
                <Card className="border-none shadow-none bg-transparent">
                  <CardHeader className="px-0 pt-0 pb-4">
                    <CardTitle className="text-[11px] font-bold uppercase tracking-[0.2em] text-muted-foreground/80 flex items-center gap-2">
                      <FileText size={13} className="text-primary/70" /> Betreff
                      & Anrede
                    </CardTitle>
                  </CardHeader>
                    ...
                    <div className="grid gap-5">
                      <div className="grid gap-2">
                        ...
                        <Input
                          ...
                          className="bg-white/80 border-primary/10 transition-all focus:border-primary/40 focus:ring-primary/10 h-11 font-medium text-sm"
                        />
                      </div>
                      <div className="grid gap-2">
                        <Label
                        ...
                        <Input
                          ...
                          className="bg-white/80 border-primary/10 transition-all focus:border-primary/40 focus:ring-primary/10 h-11 text-sm"
                        />
                      </div>
                    </div>
                  </CardContent>
                ...
                <div className="space-y-6">
                  ...
                  {data.paragraphs.map((para, i) => (
                    <div
                      key={i}
                      className="group relative grid gap-2.5 p-4 rounded-xl border border-primary/5 bg-white/40 hover:bg-white/70 transition-all"
                    >
              ...
              <TabsContent value="sender" className="m-0 space-y-8">
                  ...
                  <div className="grid gap-4 p-5 rounded-2xl border bg-white/40 shadow-sm">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <Button
                        variant="ghost"
                        size="sm"
                        onClick={handleSyncFromDeckblatt}
                        className="h-8 text-[10px] gap-1.5 text-primary hover:bg-primary/5"
                      >
                        <RefreshCw size={12} /> Sync Deckblatt
                      </Button>
                    </div>
                    <div className="grid gap-4">
                      ...
                      <div className="grid gap-1.5">
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            update("senderStreet", e.target.value)
                          }
                          className="bg-white h-10 border-slate-200"
                        />
                      </div>
                      ...
                      <div className="grid gap-1.5">
                        <Label className="text-[10px] font-bold text-slate-500 uppercase">
                          Name im Abschluss (Closing)
                        </Label>
                        <Input
                          value={data.senderNameClosing}
                          onChange={(e) =>
                            update("senderNameClosing", e.target.value)
                          }
                          className="bg-white h-10 border-slate-200"
                        />
                      </div>
                    ...
                    <div className="grid gap-4">
                      <div className="grid gap-1.5">
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            update("recipientCompany", e.target.value)
                          }
                          className="bg-white h-10 border-slate-200"
                        />
                      </div>
                      <div className="grid gap-1.5">
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            update("recipientDepartment", e.target.value)
                          }
                          className="bg-white h-10 border-slate-200"
                        />
                      </div>
                      <div className="grid gap-1.5">
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            update("recipientStreet", e.target.value)
                          }
                          className="bg-white h-10 border-slate-200"
                        />
                      </div>
                      <div className="grid gap-1.5">
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            update("recipientCity", e.target.value)
                          }
                          className="bg-white h-10 border-slate-200"
                        />
                      </div>
        ...
        <div className="lg:col-span-7 bg-[#525659] flex flex-col order-1 lg:order-2 border-b lg:border-b-0 shadow-inner overflow-hidden">
          <div className="sticky top-0 z-40 flex items-center justify-between px-4 lg:px-6 py-3 bg-black/50 backdrop-blur-md text-white/95 border-b border-white/10 shadow-xl shrink-0">
            <div className="flex items-center gap-3">
              ...
              <span className="text-[10px] xs:text-xs font-medium text-white/40 border-l border-white/10 pl-3">
                A4 Preview
              </span>
            </div>
          </div>
      ...
      <Dialog open={isQuickEditOpen} onOpenChange={setIsQuickEditOpen}>
        <DialogContent className="w-[95vw] max-w-xl p-0 overflow-hidden border-none shadow-2xl rounded-2xl sm:w-full">
          <div className="bg-linear-to-br from-primary/10 via-background to-background p-4 sm:p-6">
            <DialogHeader className="pb-4 sm:pb-6 text-left">
              <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 mb-2">
                ...
                <div>
                  <DialogTitle className="text-lg sm:text-xl font-black tracking-tight underline decoration-primary/30 underline-offset-4 line-clamp-1">
                    Smart Quick-Edit
                  </DialogTitle>
            ...
            <div className="grid gap-4 sm:gap-8 py-2 max-h-[60vh] overflow-y-auto px-1">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="grid gap-2 group">
                  <Label className="text-[10px] items-center font-black text-muted-foreground group-focus-within:text-primary transition-all uppercase tracking-widest">
                    Target Company
                  </Label>
                  <Input
                    value={data.recipientCompany}
                ...
                <div className="grid gap-2 group">
                  <Label className="text-[10px] font-black text-muted-foreground group-focus-within:text-primary transition-all uppercase tracking-widest">
                    Contact Person
                  </Label>
                  <Input
                    value={data.recipientDepartment}
                    onChange={(e) => {
                      ...
                      if (val.includes("Herr")) {
                        ...
                        !val ||
                        val.toLowerCase().includes("personal")
                      ) {
              ...
              <div className="p-3 sm:p-4 rounded-xl bg-primary/5 border border-primary/10 text-[11px] sm:text-xs text-primary/80 font-medium leading-relaxed">
                💡 <strong>Tipp:</strong> Die Anrede wird automatisch angepasst,
                wenn du "Herr Müller" oder "Frau Schmidt" eingibst.
              </div>
            </div>
            <DialogFooter className="pt-6 sm:pt-8 border-t mt-4 gap-2 sm:gap-3 flex-col sm:flex-row">
              <Button
                ...
                className="rounded-full px-6 w-full sm:w-auto h-10 sm:h-auto"
              >
                Abbrechen
              </Button>
              <Button
                onClick={() => setIsQuickEditOpen(false)}
                className="rounded-full px-8 shadow-lg shadow-primary/20 w-full sm:w-auto h-10 sm:h-auto"
              >
